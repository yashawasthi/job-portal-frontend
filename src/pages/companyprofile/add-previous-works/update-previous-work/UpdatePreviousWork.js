...
import "./UpdatePreviousWork.css"

const UpdatePreviousWork = () => {
  ...
  const savePreviousWork = async e => {
    ...
    if (title && description && startingYear && endingYear) {
      try {
        const response = await CompanyServices.updatePreviousWork(title,description,startingYear,endingYear);
...
export default UpdatePreviousWork